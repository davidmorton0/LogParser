...
class PerformanceTest < Minitest::Test
  def test_parses_a_large_log_file
    ...
    parser = Parser.new(log_reader: LogReader.new(
      options: { file_list: [file], ip_validation: :ip4_ip6 }
    ).load_logs)
    parser.count_views
    ...
    puts format('\nTook %<time>0.3f seconds to parse 10000 logs in 1 file.' \
  ...
  def test_parses_a_small_file_100_times
    ...
    begin
      ...
      time_taken = finish_time.to_f - start_time.to_f
      rate = 10_000 / time_taken
      puts format('Took %<time>0.3f seconds to parse 100 logs a 100 times.' \
                  '  %<rate>d logs / second', time: time_taken, rate: rate)
    rescue
