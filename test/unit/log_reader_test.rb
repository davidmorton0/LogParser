...
class LogReaderTest < Minitest::Test
  def test_reads_log_from_file
    log_reader = LogReader.new(
      options: { file_list: [File.join(File.dirname(__FILE__),
                                       '../test_logs/test.log')] }
    ).load_logs
  ...
  def test_counts_logs_read_from_file
    log_reader = LogReader.new(
      options: { file_list: [File.join(
        File.dirname(__FILE__), '../test_logs/test.log'
      )] }
    ).load_logs
  ...
  def test_counts_logs_read_from_multiple_files
    ...
    log_reader = LogReader.new(options: { file_list: [file1, file2] }).load_logs
