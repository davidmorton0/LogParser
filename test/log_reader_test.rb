...
class LogReaderTest < Minitest::Test
  ...
  def test_adds_log
    log_reader = LogReader.new(file: nil)
  ...
  def test_adds_only_valid_logs_read_from_string
    ...
    log_reader = LogReader.new(log_remove: true)
    assert_equal 0, log_reader.logs_read
  ...
  def test_counts_valid_logs_and_invalid_logs_read_from_file
    log_reader = LogReader.new(file: File.join(File.dirname(__FILE__), '/test_logs/ip4_validation.log'), log_remove: true)
