class WarningHandler
  ...
  def output_warnings(warnings:, add_color: false)
    warnings.map{ |warning|
      colorize_if("%<name>s: %<message>s" % {
        name: warning_info[warning[:type]][:name],
        message: warning[:message]},
        WARNING_COLORS[warning_info[warning[:type]][:important]], add_color)
    }
  end

  def important_warnings(add_color: false)
    output_warnings(warnings: @warnings.filter{ |warning|
      warning_info[warning[:type]][:important] }, add_color: add_color)
  end

  def full_warnings(add_color: false)
    output_warnings(warnings: @warnings, add_color: add_color)
  end
